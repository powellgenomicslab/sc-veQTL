...
mean_var_dir <- "/share/ScratchGeneral/angxue/proj/vQTL/mean_var_matrix"
gene_location_dir <- "/directflow/SCCGGroupShare/projects/angxue/data/onek1k/Gene_Location_Files"

# Read pseudo-bulk matrix
mx = fread(file.path(mean_var_dir, "cell_5", paste0(ct_name,"_cells_mean_mx.txt")), header = T)
mx = as.data.frame(mx)
...
mx = fread(file.path(mean_var_dir, "cell_5", paste0(ct_name,"_cells_var_mx.txt")), header = T)
mx = as.data.frame(mx)
...
gene = read.table(file.path(mean_var_dir, paste0(ct_name,"_cells_gene_list.txt")), header = T)
