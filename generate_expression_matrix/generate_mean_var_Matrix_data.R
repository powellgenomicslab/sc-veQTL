...
print(summary(as.numeric(table(as.character(info$individual)))))
# Use the individual with most cells as matrix format reference
...
# Remove genes with >90% zero variance estimate

# Remove genes with >20% non-zero similar variance estimate
...
if(all(colnames(var_mx)==colnames(co)[-1])){print("Column match checked!")}

## Save results
write.table(mean_mx,paste0(ct_name,"_cells_mean_mx.txt"),row.names=F,col.names=T,quote=F)
write.table(var_mx,paste0(ct_name,"_cells_var_mx.txt"),row.names=F,col.names=T,quote=F)
# write.table(var_std_mx,"var_std_mx.txt",row.names=F,col.names=T,quote=F)
